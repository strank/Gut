...
class TestArrayDiff:
	...
	func test_diff_with_dictionaries_fails_when_not_same_reference_but_same_values():
		...
		assert_false(diff, diff.summarize())
...
class TestArrayDeepDiff:
	...
	func test_when_deep_compare_non_equal_dictionaries_do_not_contain_disclaimer(): # TODO: outdated, dicts do not compare by ref anymore
		var a1 = [[{'a': 2}], [{'b': 3}], [{'c': 4}]]
		var a2 = [[{'a': 1}], [{'b': 2}], [{'c': 2}]]
		var diff = DiffTool.new(a1, a2, _utils.DIFF.DEEP)
